<script>
  import State from 'activestate';
  
  import { convert } from '~/lib/util.svelte';
  import { modal } from '~/lib/ui/Modal.svelte';

  const {
    total_earnings,
    from_dividends,
    from_commissions
  } = $props();

  function formatNumber(num) {
    return new Intl.NumberFormat('en-US', {
      style: 'decimal',
      minimumFractionDigits: 0,
      maximumFractionDigits: 3
    }).format(num);
  }
</script>

<main>
  <section aria-labelledby="total-earnings">
    <div class="card shimmer">
      <h1 id="total-earnings">Total Earnings</h1>
      <p class="amount">{formatNumber(total_earnings)} Satoshi</p>
      <p class="reference">~{formatNumber(convert(total_earnings, 'sat', 'eur'))} EUR</p>
      <div class="flex">
        <button class="btn secondary" onclick={modal('/dashboard/wallets/btc')}>
          <div class="i-bitcoin-icons:receive-outline"></div>
          Receive
        </button>
        <div class="btn secondary">
          <div class="i-bitcoin-icons:send-outline"></div>
          Send
        </div>
        <button class="btn secondary" onclick={modal('/dashboard/referrals/new')}>
          <div class="i-bitcoin-icons:plus-outline"></div>
          Earn
        </button>
      </div>
    </div>

    <div class="flex">
      <div class="grow">
        From Dividends
      </div>
      <div>
        {from_dividends} Satoshi
      </div>
    </div>
    <div class="flex">
      <div class="grow">
        From Commissions
      </div>
      <div>
        {from_commissions} Satoshi
      </div>
    </div>
  </section>

  <section>
    <h2>Recent Transactions</h2>
    <ul>
      {#each State.transactions as transaction}
        <li class="card">
          <h1>{transaction.type}</h1>
          <p class="amount">{formatNumber(transaction.amount)} {transaction.symbol}</p>
          <p class="reference">~{formatNumber(convert(transaction.amount, 'btc', 'eur'))} EUR</p>
        </li>
      {/each}
    </ul>
  </section>
</main>

<style>
  .card {
    padding: 1rem;
    box-shadow: 0 2px 8px rgba(80, 120, 180, 0.08);
    border-radius: 1rem;
    border: 1px solid #b0bec5;
    position: relative;
    color: black;
    line-height: 1.2;
    &.shimmer {
      background: conic-gradient(from 225deg at 55% 80%, #a9bbc5 10%, #e3f2fd 50%, #a9bbc5 50%, #e3f2fd 60%);
    }
  }

  h1 {
    font-size: 1.2rem;
    font-weight: normal;
    margin-bottom: 1rem;
  }

  .amount {
    font-size: 2rem;
  }
  .reference {
    font-size: 0.8rem;
    color: #546e7a;
    margin-bottom: 1rem;
  }
</style>